            ...
            if getattr(gpc.config.parallel.pipeline, "batch_p2p_comm", False) is True:
                for req in reqs:
                    ...
                    while not req.is_completed():
                        ...
                        if time.perf_counter() - start > TIMEOUT:
                            #print("p2p_wating > TIMEOUT1")
                            if func(steps=steps,step_id=step_id,local_pre_fetch_w=local_pre_fetch_w):
                                start = time.perf_counter()
                                #print("prefetch successful1")
                            else:
                                break
                        else:
                            #print("p2p_wating sleep1")
                            time.sleep(0.001)
                    req.wait()
                    # #print("wait success1")
                internlm_accelerator.synchronize()
                ...
                for req in ops:
                    ...
                    while not req.is_completed():
                        #print("p2p_wating2")
                        if time.perf_counter() - start > TIMEOUT:
                            #print("p2p_wating > TIMEOUT2")
                            if func(steps=steps,step_id=step_id,local_pre_fetch_w=local_pre_fetch_w):
                                start = time.perf_counter()
                                #print("prefetch successful2")
                            else:
                                break
                        else:
                            #print("p2p_wating sleep2")
                            time.sleep(0.001)
                    req.wait()
                    #print("wait success2")
    if recv_prev and recv_prev_split:
