...
class PipelineScheduler(BaseScheduler):
    ...
    def _forward_step(
        ...
        # Collect packed-sequence info (cu_seqlens) for profiling
        if getattr(gpc.config, "profile_fwd_bwd", False):
            if not hasattr(self, "_micro_batch_seq_info"):
                self._micro_batch_seq_info = []
            if isinstance(micro_batch_data, dict) and "cu_seqlens" in micro_batch_data:
                cu_seqlens_raw = micro_batch_data["cu_seqlens"]
                seq_lengths = []
                try:
                    ...
                    pass
                all_lengths = [l for s in seq_lengths for l in s]
                self._micro_batch_seq_info.append({
                    "num_sequences": len(all_lengths),
                    "seq_lengths": seq_lengths,
                    "max_seq_len": max(all_lengths) if all_lengths else 0,
                    "min_seq_len": min(all_lengths) if all_lengths else 0,
                    "mean_seq_len": sum(all_lengths) / len(all_lengths) if all_lengths else 0,
        ...
        if gpc.config.get("HETER",False) and gpc.get_global_rank() in gpc.config.get("HETER_GLOBAL_RANKS",[]):
            busy_wait_kernel(int(gpc.config["SLEEP_TIME"][0]*gpc.config["layer_partition"][gpc.get_local_rank(ParallelMode.PIPELINE)]*gpc.config["slow_ratio_dict"][gpc.get_global_rank()]))
    ...
    def _backward_step(self, engine, step_id, input_obj, output_obj, output_obj_grad, moe_loss=None, moe_z_loss=None, dp_size=1, is_last_backward=None):
        ...
        with torch.profiler.record_function(f"SCH-backward_step-{step_id}"):
            ...
            if gpc.config.get("HETER",False) and gpc.get_global_rank() in gpc.config.get("HETER_GLOBAL_RANKS",[]):
                busy_wait_kernel(int(gpc.config["SLEEP_TIME"][1]*gpc.config["layer_partition"][gpc.get_local_rank(ParallelMode.PIPELINE)]*gpc.config["slow_ratio_dict"][gpc.get_global_rank()]))
