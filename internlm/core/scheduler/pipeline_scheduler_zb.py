...
class ZeroBubblePipelineScheduler(PipelineScheduler):
    ...
    def _forward_backward_step(self, engine, return_loss=True, return_output_label=True):
        ...
        if getattr(gpc.config, 'profile_fwd_bwd', False) and os.environ.get("CUDA_LAUNCH_BLOCKING") == "1" and gpc.get_local_rank(ParallelMode.DATA) == 0 and gpc.get_local_rank(ParallelMode.TENSOR) == 0:
                ...
                with open(output_file, 'r') as f:
                    try:
                        ...
                        if "iterations" in old_data:
                            ...
                            # Migrate old format
                            for key in ["fwd_times", "bwd_times", "wwd_times"]:
                                if key in old_data:
            ...
            history["wwd_times"].append(wwd_times)
            ...
            all_wwd = [t for iteration in history["wwd_times"] for t in iteration]
            data["all_avg_wwd"] = sum(all_wwd) / len(all_wwd) if all_wwd else 0
            data["f_b_w_ratio"] = (1.0, round(data["all_avg_bwd"]/data["all_avg_fwd"], 3) if data["all_avg_fwd"] > 0 else 0, round(data["all_avg_wwd"]/data["all_avg_fwd"], 3) if data["all_avg_fwd"] > 0 else 0)
