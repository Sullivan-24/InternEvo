...
class GQA(nn.Module):
    ...
    def __init__(
        ...
        if not is_using_isp():
            self.num_kv_heads = max(num_kv_heads, gpc.get_world_size(ParallelMode.TENSOR))
        ...
        if enable_qkv_fusion:
            self.wqkv = new_linear("wqkv", embed_dim, self.q_dim + 2 * self.kv_dim, bias, **factory_kwargs)
        else:
            self.wq = new_linear("wq", embed_dim, self.q_dim, bias, **factory_kwargs)
            self.wk = new_linear("wk", embed_dim, self.kv_dim, bias, **factory_kwargs)
            self.wv = new_linear("wv", embed_dim, self.kv_dim, bias, **factory_kwargs)
        ...
        if enable_qk_norm:
            if gpc.is_rank_for_log():
                logger.info("Enable qk norm.")
            self.q_norm = new_layer_norm(qk_norm_type, self.q_dim, eps=layer_norm_epsilon)
            self.k_norm = new_layer_norm(qk_norm_type, self.kv_dim, eps=layer_norm_epsilon)
        ...
        self.inner_attn = GroupQueryAttention(
            inner_attn, process_group=gpc.get_group(ParallelMode.GQA) if self.need_dkv_reduction else None
        )
        self.inner_cross_attn = GroupQueryAttention(
            inner_cross_attn, process_group=gpc.get_group(ParallelMode.GQA) if self.need_dkv_reduction else None
        )

        self.wo = new_linear("wo", self.q_dim, embed_dim, not wo_no_bias if bias else False, **factory_kwargs)
    ...
    def _apply_qkv_transform(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
...
class GlobalKVCrossAttention(nn.Module):
    """
    Cross-attention with global kv. Usually used in `InternLM3CrossDecoder`. Currently do not support window_size.
    ...
    def forward(self, x, kv, inference_params=None, **kwargs):
        if inference_params is None:
            return self._training(x=x, kv=kv, **kwargs)
        else:
            return self._inference(x=x, kv=kv, inference_params=inference_params, **kwargs)

    def _training(self, x, kv, **kwargs):
        """
        Arguments:
            x: (batch, seqlen, hidden_dim)
            kv：(batch, seqlen, 2, num_heads, head_dim)
        """
        q = self.wq(x)
        q = rearrange(q, "b s (h d) -> b s h d", d=self.head_dim)
    ...
    def _inference(self, x, kv, inference_params, **kwargs):  # pylint: disable=W0613
        """
        Arguments:
            x: (batch, seqlen, hidden_dim)
            kv：(batch, total_seqlen, 2, num_heads, hidden_dim) Note that kv is already cached and concatenated.
        """
        assert inference_params is not None, "inference_params is required for inference"
        assert self.layer_idx is not None, "Generation requires layer_idx in the constructor"
        attention_mask = inference_params.attention_mask
...
class GroupQueryAttention(nn.Module):
    ...
    def _backward_hook(self, module, grad_input, grad_output):  # pylint: disable=W0613
        ...
        if internlm_accelerator.get_accelerator_backend() in [
            ...
            dkv.div_(len(dist.dist.get_process_group_ranks(self.proccess_group)))
        else:
