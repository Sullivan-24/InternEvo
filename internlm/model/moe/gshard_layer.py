...
class GShardMoELayer(BaseMoELayer):
    ...
    def forward(self, *inputs: Tensor) -> Tensor:
        ...
        if not self.enable_token_rearrange_opt:
            self.l_aux, combine_weights, dispatch_masks, self.z_loss = self.gate(reshaped_inputs, inputs[1])
            dispatched_inputs = einsum(
                "sec,sm->ecm", dispatch_masks.type_as(inputs[0]), reshaped_inputs
            )  # TODO: heavy memory usage due to long sequence length
        else:
            self.l_aux, token_rearrange_infos, self.z_loss = self.gate(reshaped_inputs)
