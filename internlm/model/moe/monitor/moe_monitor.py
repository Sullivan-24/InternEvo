...
from internlm.model.moe.utils import gather_along_first_dim_expert_parallel
...
class MoEMonitor(BaseMonitor):
    ...
    def expert_profiling_wait(self):
        ...
        if batch_count % self.interval_steps == 0:
            if self.tokens_above_avg_monitor:
                if self.handles:
                    self.clear_handles()
                gpc.metrics["tokens_above_avg_max"].append(self.tokens_above_avg_max)
                gpc.metrics["tokens_above_avg_min"].append(self.tokens_above_avg_min)
            if self.expert_activation_monitor:
                if self.handles:
                    self.clear_handles()
                gpc.metrics["expert_activation"].append(self.expert_activation)
    ...
    def pre_gate_profiling(self, inputs: Tensor):
        if self.get_logit_before_gate and gpc.config.get("batch_count") % self.interval_steps == 0:
            with torch.no_grad():
                gpc.metrics["logit_before_gate_max"].append(inputs.max())
                gpc.metrics["logit_before_gate_min"].append(inputs.min())
                gpc.metrics["logit_before_gate_mean"].append(inputs.mean())
