...
def order_result_mutichunk(input: str, stage_placement: list, num_microbatches:int, dp_size:int, pp_size:int) -> None:
    ...
    return all_rank_workloads,recomp_stages, max_end_time
    ...
    for dp_rank,comm_graph_ in enumerate(comm_graph):
        for pp_rank, ops in enumerate(comm_graph_):
            ...
            for workload_index, comms in enumerate(ops):
                for comm in comms:
                    op_type, _, match_dp_rank, match_pp_rank, stage_id, chunk_id, microbatch_id, match_workload_index = comm
...
def generate_comm_graph(comp_graph, stage_placement, max_end_time, send_immediately, dp_size, pp_size, transfer_info):
                        ...
                        if time >= end_time:
                            ...
                            if (workload_type == WorkloadType.FORWARD.value and stage_id<max_stage_id) or (workload_type == WorkloadType.BACKWARD.value and stage_id>min_stage_id):
                                ...
                                if recv_dp_rank is not None and dst_pp_rank is not None:
                                    ...
                                    if workload_type == WorkloadType.FORWARD.value:
                                        recv_end_index = search_by_infor(recv_workloads,workload_type,microbatch_id,stage_id+1,source_dp_rank)+1
                                    elif workload_type == WorkloadType.BACKWARD.value:
                                        recv_end_index = search_by_infor(recv_workloads,workload_type,microbatch_id,stage_id-1,source_dp_rank)+1
                                    #确定接收的pp_rank有哪些接收区间
                                    ...
                                    if workload_type == WorkloadType.FORWARD.value:
                                        comm_graph[recv_dp_rank][dst_pp_rank][recv_location].append(('RA', end_time, dp_rank, pp_rank, stage_id, chunk_id, microbatch_id, send_location+1))
                                        comm_graph[dp_rank][pp_rank][send_location+1].append(('SA', end_time,recv_dp_rank, dst_pp_rank, stage_id, chunk_id, microbatch_id,recv_location))   
                                    # comm_graph[pp_rank][send_location]['S'].append((op, _, dst_pp_rank, stage_id, chunk_id, microbatch_id,_))
                                    elif workload_type == WorkloadType.BACKWARD.value:
                                        comm_graph[recv_dp_rank][dst_pp_rank][recv_location].append(('RG', end_time, dp_rank, pp_rank, stage_id, chunk_id, microbatch_id,send_location+1))
                                        comm_graph[dp_rank][pp_rank][send_location+1].append(('SG', end_time, recv_dp_rank, dst_pp_rank, stage_id, chunk_id, microbatch_id,recv_location)) 
                        break
                            # recv_info = {}
