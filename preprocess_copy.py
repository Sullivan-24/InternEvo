...
def align(ops, index, len_ops, align_lengeth):
    
    for i in range(index,len_ops):
    ...
    for device_id, stage_ops in enumerate(grouped_data):
        ...
        for m, current_op in enumerate(stage_ops):
            ...
            for i in range(len(needrecv['F_stage'])):
                ...
                while(recv_f_pointer[i]<len_ops):
                    n = recv_f_pointer[i]
                ...
                #for n, prev_op in enumerate(prev_stage_ops):
                    ...
                    if op == 'f' and prev_microbatch_id == microbatch_id and prev_stage_id == stage_id-1:
                        ...
                        if interval_distance > 0:
                            align(stage_ops,m,len_ops,interval_distance)
                        elif interval_distance < 0 :
                            if n+1 < len_ops:
                                align(prev_stage_ops,n+1,len_ops,-interval_distance)
                    ...
                    if min_dist == interval_before_recv_distance_:
                        if interval_before_recv_distance > 0:
                            align(stage_ops,m,len_ops,interval_before_recv_distance_)
                        elif interval_before_recv_distance < 0 :
                            if n+1 < len_ops:
                                align(prev_stage_ops,n+1,len_ops,interval_before_recv_distance_)
                        comm_op['B'].append(('f', prev_end_time, recvFdevice_id, prev_stage_id,prev_chunk_id, prev_microbatch_id, n, 0))
                        ...
                        if interval_after_recv_distance > 0:
                            if m+1 < len_ops:
                                align(stage_ops,m+1,len_ops, interval_after_recv_distance_)
                        elif interval_after_recv_distance < 0:
                            if n+1 < len_ops:
                                align(prev_stage_ops,n+1,len_ops,interval_after_recv_distance_)
                        comm_op['A'].append(('f',prev_end_time, recvFdevice_id, prev_stage_id,prev_chunk_id, prev_microbatch_id, n, 0))
                        recv_f_pointer[i] += 1  # 标记为已接收
                        continue
                    else:
            ...
            for j in range(len(needrecv['B_stage'])):
                ...
                while(recv_b_pointer[j]<len_ops):
                    n = recv_b_pointer[j]
                ...
                #for n, next_op in enumerate(next_stage_ops):
                    ...
                    if op == 'b' and next_microbatch_id == microbatch_id and next_stage_id == stage_id+1:
                        ...
                        if interval_distance > 0:
                            ...
                            if n+1 < len_ops:
                                align(next_stage_ops,n+1,len_ops,-interval_distance)                        
                    ...
                    if min_dist == interval_before_recv_distance_:
                        if interval_before_recv_distance > 0:
                            ...
                            if n+1 < len_ops:
                                align(next_stage_ops,n+1,len_ops,interval_before_recv_distance_)
                        comm_op['B'].append(('b', next_end_time, recvBdevice_id, next_stage_id,next_chunk_id, next_microbatch_id, n, 0))
                        ...
                        if interval_after_recv_distance > 0:
                            ...
                            if n+1 < len_ops:
                                align(next_stage_ops,n+1,len_ops,interval_after_recv_distance_)
                        comm_op['A'].append(('b',next_end_time, recvBdevice_id, next_stage_id,next_chunk_id, next_microbatch_id, n, 0))
                        recv_b_pointer[j] += 1  # 标记为已接收
                        continue
                    else:
...
if __name__ == '__main__':
    stage_alignment, unified_scheduler, comm_graph = generate_Interleaved_4pp_20chunk_16mb()
