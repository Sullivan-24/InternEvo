...
from internlm.model.modules.linear import ColumnParallelLinear, RowParallelLinear
from internlm.model.ops.rotary_emb import apply_rotary_emb
from internlm.model.ops._flash_attn import FlashAttnVarlenKVPackedFunc_V263
from internlm.model.ops.attention import _flash_varlen_kvpacked_attn, SelfAttention
from internlm.model.ops.utils import unpack_qkv_before_attn
import internlm.model.ops.rotary_emb
import internlm.model.ops.attention
import einops
...
class FlopsProfiler(object):
    ...
    def start_profile(self, ignore_list=None):
        ...
        def register_module_hooks(module, ignore_list):
            ...
            #         module.__flops_handle__ = module.register_forward_hook(_self_attention_forward_hook)
            #     return
            ...
            if type(module) in MODULE_HOOK_MAPPING:
                ...
                if type(module) in (nn.RNN, nn.GRU, nn.LSTM, nn.RNNCell, nn.LSTMCell, nn.GRUCell):
                    return
...
def _column_linear_flops_compute(input, out_features, bias=None): 
    macs = input.numel() * out_features
    return 2 * macs, macs

def _row_linear_flops_compute(input, out_features, bias=None):
    macs = input.numel() * out_features
    return 2 * macs, macs
...
# def unpack_qkv(x, cu_seqlens):
    
#     x = x.squeeze(dim=0)
#     unpack_sequences = []
#     for i in range(len(cu_seqlens) - 1):
#         varlen_sequence = x[ cu_seqlens[i]: cu_seqlens[i + 1] ]
#         unpack_sequences.append(varlen_sequence)
        
#     return unpack_sequences


# def _flash_attn_flops_compute(q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, *args, **kwargs):    
#     """
#     Count flops for the _flash_varlen_kvpacked_attn operation.
#     """
#     # shape (1, packlen, num_packed, head_nums, head_dim)
#     # q, kv = q.squeeze(dim=0), kv.squeeze(dim=0)
    
#     # pack length = total sequence length
#     assert q.shape[1] == kv.shape[1]
        
#     # shape(micro_bsz, packed_length, num, num_heads, head_dim)
#     q = unpack_qkv(q, cu_seqlens=cu_seqlens_q)
#     kv = unpack_qkv(kv, cu_seqlens=cu_seqlens_k)
    
#     k, v = kv[:, :, 0], kv[:, :, 1]
    
#     # macs for score, compute from one head
#     macs = len(q) * q[0].numel * k.shape[1]
    
#     # macs for soft_max shape(micro_bsz, seq_len_q, num_heads, seq_len_k)
#     macs += 3 * len(q) * q[0].numel * k.shape[1] / q.shape[-1]
    
#     # macs for weighted sum
#     macs += len(q) * q[0].numel * v.shape[-1]
...
def _flash_attn_flops_compute(q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, *args, **kwargs):    
    ...
    Count flops for the _flash_varlen_kvpacked_attn operation.
    """
    # q shape: (1, total_q, n_head, head_dim) (packed shape)
    import pdb; pdb.set_trace()
    n_head = q.shape[2]
    head_dim = q.shape[3]
    
    # Calculate sequence lengths from cumulative sequence lengths
    # cu_seqlens is int32, convert to float for calculation
    seq_len_q = (cu_seqlens_q[1:] - cu_seqlens_q[:-1]).float()
    seq_len_k = (cu_seqlens_k[1:] - cu_seqlens_k[:-1]).float()
    
    # macs for score Calculate total valid interactions: sum(seq_len_q[i] * seq_len_k[i])
    total_interactions = torch.dot(seq_len_q, seq_len_k).item()
    
    # Total MACs = Score Calculation (Q * K^T) + Output Calculation (Score * V)
    # Each part is: total_interactions * n_head * head_dim
    macs = 2 * total_interactions * n_head * head_dim + 3 * total_interactions * n_head
    
    return int(2 * macs), int(macs)
...
def _apply_emb_flops_compute(x,*args, **kwargs):
    """
    Count flops for the apply_rotary_emb operation.
    """
    return 3 * x.numel(), 0
...
def _patch_functionals():
    ...
    global _flash_varlen_kvpacked_attn
    ...
    # internlm.model.ops.rotary_emb.apply_rotary_emb = wrapFunc(internlm.model.ops.rotary_emb.apply_rotary_emb, _apply_emb_flops_compute)
    ...
    # dropout
    F.dropout = wrapFunc(F.dropout, _dropout_flops_compute)
...
def _patch_tensor_methods():
    ...
    torch.softmax = wrapFunc(torch.softmax, _softmax_flops_compute) 
...
def _reload_functionals():
    ...
    # internlm.model.ops.rotary_emb.apply_rotary_emb = old_functions[internlm.model.ops.rotary_emb.apply_rotary_emb.__str__]
...
def _self_attention_forward_hook(module, input, output):
    """
    Hook to estimate Flash Attention FLOPs based on input arguments in SelfAttention.forward.
    Expected input signature matches SelfAttention.forward:
    (q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, ...)
    """
        # input is a tuple of arguments passed to forward
        # We assume the order matches: q, kv, cu_seqlens_q, cu_seqlens_k, ...
        # Based on InternLM implementation of SelfAttention
    # import pdb; pdb.set_trace()
    if len(input) >= 4:
        q = input[0]
        # kv = input[1] 
        cu_seqlens_q = input[2]
        cu_seqlens_k = input[3]

        if torch.is_tensor(q) and torch.is_tensor(cu_seqlens_q) and torch.is_tensor(cu_seqlens_k):
            # q shape: (1, packed_seq_len, n_heads, head_dim) or similar
            # We need n_heads and head_dim
            if q.dim() >= 4:
                n_head = q.shape[2]
                head_dim = q.shape[3]
            else:
            ...
            # Calculate sequence lengths from cumulative sequence lengths
            # cu_seqlens is int32, convert to float for dot product
            seq_len_q = (cu_seqlens_q[1:] - cu_seqlens_q[:-1]).float()
            seq_len_k = (cu_seqlens_k[1:] - cu_seqlens_k[:-1]).float()

            # Calculate total valid interactions in the batch: sum(seq_len_q[i] * seq_len_k[i])
            total_interactions = torch.dot(seq_len_q, seq_len_k).item()

            # MACs Calculation for Attention:
            # 1. Q * K^T -> Score: total_interactions * n_head * head_dim
            # 2. Score * V -> Output: total_interactions * n_head * head_dim
            # Total MACs = 2 * total_interactions * n_head * head_dim
            
            macs = 2 * total_interactions * n_head * head_dim + 3 * total_interactions * n_head # including softmax ops
...
def _column_linear_forward_hook(module, input, output):
    """Count flops for ColumnParallelLinear."""
    input_tensor = input[0] if isinstance(input, tuple) else input
    out_features = module.out_features
    macs = input_tensor.numel() * out_features
    module.__flops__ += int(2 * macs)
    module.__macs__ += int(macs)

def _row_linear_forward_hook(module, input, output):
    """Count flops for RowParallelLinear."""
    input_tensor = input[0] if isinstance(input, tuple) else input
    out_features = module.out_features
    macs = input_tensor.numel() * out_features
    module.__flops__ += int(2 * macs)
...
# def _flash_attn_263_forward_hook(module, input, output):
#     """Count flops for FlashAttention 2.6.3+."""
...
MODULE_HOOK_MAPPING = {
    ...
    ColumnParallelLinear: _column_linear_forward_hook,
    RowParallelLinear: _row_linear_forward_hook,
    # SelfAttention: _self_attention_forward_hook,
}
